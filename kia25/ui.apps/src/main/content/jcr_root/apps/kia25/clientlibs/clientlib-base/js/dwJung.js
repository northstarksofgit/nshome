...
var showBtnActMove = function() {
  ...
  , offersInit = function() {
    var c = new OpenDimPop($("#offersLayer"));
    if (0 < $("div.sales_offers").length)
        $("a.carPop").on("click", function(a) {
            a.stopPropagation();
            a.preventDefault();
            c.open()
        })
  ...
  , buildcarInit = function() {
    function c() {
        if (l.length) {
            var a = l.attr("title")
              , c = a.slice(a.lastIndexOf(" ") + 1, a.length);
            a = f.hasClass("on") ? a.replace(c, "\ub2eb\uae30") : a.replace(c, "\uc5f4\uae30");
            l.attr("title", a)
        }
    }
    function a() {
        var c = $(window).scrollTop() + $(window).height()
          , f = $(".build_graph").offset().top + 100;
        c > f && ($(window).off("scroll", a),
        $(".build_graph .graph").each(function(a, c) {
            a = $(this).find(".rate .num").text();
            c = $(this).height();
            e(this, c, a)
        }))
    }
    function e(a, c, e) {
        function f() {
            ...
            a = Math.round(e - Math.max((h - a) / 1E3, 0) * l);
            g(a);
            a == e && clearInterval(B)
        }
        function g(a) {
            k.css("background-position", "0px " + c * -a + "px")
        }
        var k = $(a)
          , l = e - 0
          , h = (new Date).getTime() + 1E3;
        var B = setInterval(f, 50);
        f();
    ...
    (function() {
        ...
        var e = $(".car_list")
          , h = $("div.category label");
        $("div.category input:checkbox").on("click", function(c) {
            c = $(this).attr("id");
            $(this).attr("name");
            c = $("label[for\x3d'" + c + "']").closest("span").index();
            var f = h.eq(c).hasClass("c_on");
            e.hide();
            6 < h.length ? f ? 0 === c ? (h.removeClass("c_on").prev().attr("checked", !1).end().eq(c).addClass("c_on"),
            0 === c ? e.not(":last").show() : e.eq(c - 1).show()) : (h.eq(0).add(h.eq(h.length - 1)).removeClass("c_on").prev().attr("checked", !1),
            a()) : a() : f ? 0 === c ? (h.removeClass("c_on").prev().attr("checked", !1).end().eq(0).addClass("c_on"),
            e.show()) : (h.eq(0).removeClass("c_on").prev().attr("checked", !1),
            a()) : a()
        });
        e.filter(":last").hide();
        h.on("click.build", function(c) {
            c = $(this).closest("span").index();
            var f = $(this).hasClass("c_on");
            e.hide();
            6 < h.length ? f ? 0 === c ? (h.removeClass("c_on").prev().attr("checked", !1).end().eq(c).addClass("c_on"),
            0 === c ? e.not(":last").show() : e.eq(c - 1).show()) : (h.eq(0).add(h.eq(h.length - 1)).removeClass("c_on").prev().attr("checked", !1),
            a()) : a() : f ? 0 === c ? (h.removeClass("c_on").prev().attr("checked", !1).end().eq(0).addClass("c_on"),
    ...
    $(".pay_guide \x3e a:eq(0)").on("click", function(a) {
        a.preventDefault();
        v.open()
    });
    $(".pay_guide \x3e a:eq(1)").on("click", function(a) {
        a.preventDefault();
        da.open()
    });
    $(".pay_guide \x3e a:eq(2)").on("click", function(a) {
        a.preventDefault();
        ea.open()
    });
    $(".pay_guide \x3e a:eq(3)").on("click", function(a) {
        a.preventDefault();
        fa.open()
    });
    $(".pay_guide \x3e a:eq(4)").on("click", function(a) {
        a.preventDefault();
        w.open()
    });
    $(".pay_guide \x3e a:eq(5)").on("click", function(a) {
        a.preventDefault();
        ca.open()
    });
    $(".disabled_btn \x3e a").on("click", function(a) {
        a.preventDefault();
        C.open()
    });
    (function() {
...
function noCarDivHide(c) {
    $(".car_u").each(function(a, e) {
        a = $(this).find("li.car_l").length;
        e = $(this).find("li.noCar").length;
        e = 0 == a % c ? a % c + e : a % c + e - c;
        $(this).find("li.noCar").show();
        for (a = 0; a < e; a++)
            $(this).find("li.noCar").eq(a).hide()
    })
}
var shoppingtoolsCategoryInit = function() {
    function c() {
        e.each(function(c) {
            $(this).hasClass("c_on") && a.eq(c - 1).show()
        })
    }
    var a = $(".car_list")
...
var subSlideTime = 500
  , timer = 0
  , complete_graphSlideInit = function(c) {
    function a(a) {
        if (null != e) {
            g = a.pageX || a.originalEvent.touches[0].pageX;
            if (e + 40 < g)
    ...
    $(".complete_graph .pip_next").on("click", function(a) {
        a.preventDefault();
        a.stopPropagation();
        var c = $(this).parents(".mInner").find(".imgList").children(".on").index();
        Math.abs(timer - $.now()) > subSlideTime && (timer = $.now(),
        a = $(this).parents(".mInner").find(".imgList").children("li").length,
        c++,
        a < c + 1 && (c = 0),
        $(this).parents(".mInner").find(".imgList").children("li").eq(c).addClass("on").siblings("li").removeClass("on"),
        subSlideTime,
        function() {
            $(this).parents(".mInner").find(".imgList").closest(".active").attr("data-len", c)
        }
        )
    });
    $(".complete_graph .pip_prev").on("click", function(a) {
        a.preventDefault();
        a.stopPropagation();
        var c = $(this).parents(".mInner").find(".imgList").children(".on").index();
        Math.abs(timer - $.now()) > subSlideTime && (timer = $.now(),
    ...
    var e, g;
    $(".complete_graph .imgList").on("touchstart", function(a) {
        a.stopPropagation();
        e = a.pageX || a.originalEvent.touches[0].pageX
    });
    $(".complete_graph .imgList").on("touchmove", function(c) {
        "prev" == a(c) ? ($(this).siblings(".pipBtnLeft").find(".pip_prev").trigger("click"),
        $(this).trigger("touchend")) : "next" == a(c) && ($(this).siblings(".pipBtnRight").find(".pip_next").trigger("click"),
        $(this).trigger("touchend"))
    });
    $(".complete_graph .imgList").on("touchend", function(a) {
        a.stopPropagation();
        e = null
    })
};
...
function updateBuildOption() {
    if (0 < $(".build_option").length) {
        ...
        for (var l = $(".build_option .box_l.on"), k = 0; k < l.length; k++) {
            ...
            h = $(h).attr("price");
            c.append("\x3cspan price\x3d" + h + '\x3e\x3cem class\x3d"conTitle"\x3e' + m + '\x3c/em\x3e\x3cspan class\x3d"conPrice"\x3e' + parseInt(h).format() + "\x3cem\x3e\uc6d0\x3c/em\x3e\x3c/span\x3e\x3c/span\x3e");
            g++;
            f += parseInt(h)
        }
        if (0 < $(".build_tuon").length)
            for (l = $(".build_tuon .box_l.on"),
            k = 0; k < l.length; k++)
                h = l[k],
                m = $(h).find("span.h").text(),
                h = $(h).attr("price"),
        ...
        updateTotalPrice();
        a.text(g);
        e.text(parseInt(f).format())
    }
}
function updateBuildMethod() {
    if (0 < $(".build_pay").length) {
        var c = $(".totalList .list04 .con_pay")
          , a = $(".totalList .list04 .con_etc");
        c.empty();
        a.empty();
        var e = $(".method .pay_u")
          , g = e.find("input:checked").attr("method")
          , f = $("input[name\x3dsample11]:checked").attr("term");
        e = 1E3 * parseInt(e.find(".initFee").val().replace(/,/g, "") || 0);
        c.append('\x3cspan\x3e\x3cem class\x3d"tit"\x3e\uc9c0\ubd88\ubc29\ubc95\x3c/em\x3e\x3cem class\x3d"txt"\x3e' + ("installment" === g ? "\uc624\ud1a0\ud560\ubd80" : "\ud604\uae08\uad6c\uc785") + "\x3c/em\x3e\x3c/span\x3e");
        "installment" === g && (c.append('\x3cspan\x3e\x3cem class\x3d"tit"\x3e\ud560\ubd80\x3c/em\x3e\x3cem class\x3d"txt"\x3e' + f + " \uac1c\uc6d4\x3c/em\x3e\x3c/span\x3e"),
        c.append('\x3cspan\x3e\x3cem class\x3d"tit"\x3e\ud560\ubd80\uc6d0\uae08\x3c/em\x3e\x3cem class\x3d"txt"\x3e' + (10 * e).format() + " \uc6d0\x3c/em\x3e\x3c/span\x3e"));
        "yes" == $("input:radio[name\x3dsample3]:checked").val() ? c.append('\x3cspan\x3e\x3cem class\x3d"tit"\x3e\ucc28\ub7c9\uac00\x3c/em\x3e\x3cem class\x3d"txt"\x3e\uba74\uc138\x3c/em\x3e\x3c/span\x3e') : c.append('\x3cspan\x3e\x3cem class\x3d"tit"\x3e\ucc28\ub7c9\uac00\x3c/em\x3e\x3cem class\x3d"txt"\x3e\uacfc\uc138\x3c/em\x3e\x3c/span\x3e');
        c = parseInt($("[name\x3d'mpoint']").val().replace(/,/g, "") || 0);
        g = parseInt($("[name\x3d'qpoint']").val().replace(/,/g, "") || 0);
        a.append('\x3cinput type\x3d"hidden" class\x3d"pointdc" price\x3d"' + (c + (2E6 < g ? 2E6 : g)) + '" /\x3e');
        c = parseInt($("[name\x3d'transfee']").attr("price") || 0);
        a.append('\x3cinput type\x3d"hidden" class\x3d"transfee" price\x3d"' + c + '" /\x3e');
        updateTotalPrice()
    }
}
Number.prototype.format = function() {
    if (0 == this)
        return 0;
    for (var c = /(^[+-]?\d+)(\d{3})/, a = this + ""; c.test(a); )
        a = a.replace(c, "$1,$2");
    return a
}
;
